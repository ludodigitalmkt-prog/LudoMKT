...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById('form-rh-completo')?.addEventListener('submit', async (e) => {
        ...
        } catch(err) { showToast("Erro: " + err.message, "#ff3366"); }
    });
    ...
    // UPLOAD CONFIGURAÇÕES (COM FEEDBACK VISUAL)
    ...
    // AGENDA E KANBAN (COM ANEXO E MÚLTIPLAS DATAS)
    ...
    onSnapshot(query(collection(db, "atividades"), orderBy("criadoEm", "desc")), (snap) => {
        ...
        snap.forEach((d) => {
            ...
            // Lógica do Kanban (Mostrar apenas hoje ou atrasados)
            ...
            // Adiciona no calendário em todas as datas correspondentes
    ...
    document.getElementById('send-ia')?.addEventListener('click', async () => {
        ...
        if(!m) return; cb.innerHTML += `<div class="msg-user">${m}</div>`; inp.value = "";

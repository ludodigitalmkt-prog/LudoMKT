...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // ==========================================
    // CARREGAR CONFIGURAÇÕES (CSS, LOGO, LINKS)
    // ==========================================
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            document.getElementById('ludotech-widget').style.display = 'block'; // IA FLUTUANTE ATIVA!
    ...
    document.getElementById('btn-sacar-vr')?.addEventListener('click', async () => {
        ...
        try {
            ...
            alert('Saque de R$' + usuarioLogado.vr_saldo + ' gerado com sucesso! Saldo zerado.');
    ...
    document.getElementById('btn-salvar-logos')?.addEventListener('click', async () => {
        ...
        if(fMain || fIa) { await setDoc(doc(db, 'configuracoes', 'geral'), d, { merge: true }); alert('Logos salvas com sucesso!'); aplicarConfiguracoes(); }
    });
    ...
    async function carregarUsuariosNoSelect() {
        ...
        sel.innerHTML = ''; if(selUpdate) selUpdate.innerHTML = '<option value="" disabled selected>Selecione...</option>';
        qs.forEach((d) => {
    ...
    // ==========================================
    // AGENDA E KANBAN (COM REGISTRO DE AÇÕES)
    // ==========================================
    ...
    // RENDERIZAR TAREFAS E RELATÓRIOS
    ...
    onSnapshot(query(collection(db, "atividades"), orderBy("criadoEm", "desc")), (snap) => {
        ...
        // Limpa variáveis de resumo na Home
        ...
        snap.forEach((d) => {
            ...
            // Calendários (Geral e Individual)
            if (data.datas && data.datas !== '') {
                ...
                evG.push(ev);
                // Fix Bug: Planner Individual só mostra as tarefas do usuário logado
    ...
    // ==========================================
    // INICIALIZADORES (CALENDÁRIOS E LUDOPLAY)
    // ==========================================
